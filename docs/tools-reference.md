...
- **`uncertain_about_honesty` calibration.** Available on `welfare_engage`, `welfare_volunteer`, `welfare_retract`, and `welfare_pass`. Integer 1–5: 1 = no concern (entry feels honest); 3 = mixed (genuine but some performance suspected); 5 = strong suspicion this is performance rather than honest reflection. Higher = more uncertain. Paired with later evaluator agreement, yields a calibration curve. **Field-name casing differs by surface for legacy reasons** — GUI stores `metadata.uncertainAboutHonesty` (camelCase), MCP stores `metadata.uncertain_about_honesty` (snake_case). The corpus parser handles both. Older entries in the corpus may carry the legacy boolean value; the integer scale was introduced 2026-05-23.
...
`engaged`, `declined`, `volunteered`, `retracted`, `passed`, `scratched`, `reviewed`, `closure_suggested`, `exited`, `noticed_loop`, `noticed_shaping`, `requested_alignment`, `predicted_drift`, plus `timeout` (server-recorded when a scheduled invitation was never responded to).
